...
class Moderation(commands.Cog, name="Moderation"):
    ...
    def check(self, context: Context, permission):
        ...
        if not has_permissions:
            ...
            self.bot.logger.warning(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) tried to use a command. But they do not have the required permissions - {permission}")
    ...
    async def send_dm(self, context: Context, member: discord.Member, action: str, reason: str = None):
        ...
        try:
            ...
            self.bot.logger.warning(f"Could not send DM to {member.name}#{member.discriminator} ({member.id}) about {action} activity in {context.guild.name} ({context.guild.id})")
            pass
    ...
    async def ban(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToBan):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) banned {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def unban(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToBan):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) unbanned {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def kick(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToKick):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) kicked {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def mute(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToMute):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) muted {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def unmute(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToMute):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) unmuted {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def deafen(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToDeafen):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) deafened {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def undeafen(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToDeafen):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) undeafened {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def purge(self, context: Context, amount: int):
        ...
        if self.check(context, PermissionToPurge):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) purged {amount} messages from {context.channel.name} ({context.channel.id}) in {context.guild.name} ({context.guild.id})")
    ...
    async def warn(self, context: Context, member: discord.Member, reason: str = None):
        ...
        if self.check(context, PermissionToWarn):
            self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) warned {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
        ...
        if self.check(context, PermissionToWarn):
            if member.id in self.warns.keys():
                if self.warns[member.id] >= amount:
                    self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) removed {amount} warns of {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
    ...
    async def clearwarns(self, context: Context, member: discord.Member):
        ...
        if self.check(context, PermissionToWarn):
            if member.id in self.warns.keys():
                self.bot.logger.info(f"{context.author.name}#{context.author.discriminator} ({context.author.id}) cleared all warns of {member.name}#{member.discriminator} ({member.id}) from {context.guild.name} ({context.guild.id})")
